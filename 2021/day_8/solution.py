...
def get_decoded_output(line):
    ...
    number_encodings[2] = next(codes[0] for codes in missing_segment_codes.values() if len(codes) == 1)

    five_six = next(codes for codes in missing_segment_codes.values() if len(codes) == 2 and number_encodings[1] not in codes)

    number_encodings[5] = next(code for code in five_six if len(code) == 5)

    number_encodings[6] = next(code for code in five_six if len(code) == 6)

    number_encodings[0] = next(codes[0]
                           for codes in
                           [[code for code in codes if code not in number_encodings.values()]
                            for seg, codes in segment_codes.items()]
                           if len(codes) == 1)

    number_encodings[3] = next(code for code in unique_codes if code not in number_encodings.values() and len(code) == 5)

    number_encodings[9] = next(code for code in unique_codes if code not in number_encodings.values() and len(code) == 6)
